...
2. Navigate to the `navigation/` folder, and run the command below to install required dependencies.
  ```
...
4. Place the file in the deeprl GitHub repository, in the `navigation/` folder, and unzip (or decompress) the file.

### Instructions

Follow the instructions in `Navigation.ipynb` to get started with training your own agent!  To use a Jupyter notebook, run the following command from the `navigation/` folder:
```
jupyter notebook
```
